...
public class UnwillingEntity extends Monster implements GeoEntity {
        ...
        if (!this.swinging) {
            if (velocity > 0.05F && this.onGround()) {
                state.getController().setAnimation(
                        RawAnimation.begin()
                                .thenLoop("animation.unwilling.walk")

                );
            } else {
                state.getController().setAnimation(
                        RawAnimation.begin()
                                .thenLoop("animation.unwilling.idle")
    ...
    private <T extends GeoAnimatable> PlayState attackPredicate(AnimationState<T> state) {
        if (this.getAttackAnim(state.getPartialTick()) > 0.0F && !this.swinging) {
            ...
            state.getController().setAnimation(
                    RawAnimation.begin()
                            .then("animation.unwilling.attack", Animation.LoopType.PLAY_ONCE)
            );
